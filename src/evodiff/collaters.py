...
class OAMaskCollaterSingleSeq(object):
    """
    OrderAgnosic Mask Collater for masking batch data according to Hoogeboom et al. OA ARDMS
    Revised by SZ to adapt to diffab. 
    ...
    outputs:
        ...
        masks: masks used to generate src
    """
    def __init__(self, mask_id:int = 22, num_steps:int = 100):
        self.mask_id = mask_id
        self.num_steps = num_steps

    def __call__(self, sequences:torch.Tensor, timesteps:torch.Tensor, seq_mask:torch.Tensor):
        """
        sequences: (N, L)
        timesteps: (N,)
        seq_mask: 1 for valid tokens and zero for padding tokens;(N, L)
        """
        #tokenized = [torch.tensor(self.tokenizer.tokenize(s)) for s in sequences]
        
        N, L = sequences.shape
        device = sequences.device
        seq_len = seq_mask.sum(dim = -1)  # (N,)

        ### num masks
        num_mask = (seq_len * timesteps / self.num_steps).round().int() # (N,)
        #* when len == num_steps, num_mask = t

        ### mask
        mask_arr = [np.random.choice(int(s_len), int(num_mask[i]), replace=False)
                       for i, s_len in enumerate(seq_len)]
        index_arr = np.arange(0, L) #index array [1...seq_len]
        masks = [np.isin(index_arr, m_arr, invert=False).reshape(index_arr.shape)
                   for m_arr in mask_arr] 
        masks = torch.tensor(masks, dtype=torch.bool).to(device)  # (N, L)

        ### masked sequence
        src = ~masks * sequences + masks * self.mask_id  # (N, L)
        src = src.to(torch.long)

        return src, num_mask, masks
...
class D3PMCollater(object):
    ...
    def __call__(self, sequences):
        ...
        for i,t in enumerate(tokenized): # enumerate over batch
            ...
            x = one_hot[i, :D, :] # self.tokenizer.one_hot(t); (N, l, token_size)
...
class D3PMCollaterSingleSeq(object):
    """
    D3PM Collater for generating batch data according to markov process according to Austin et al.
    masking scheme: 'BLOSUM' uses blosum matrix, 'RANDOM' uses uniform transition matrix
    ...
    Revised by SZ to adapt to diffab.
    """
    def __init__(self, token_size, Q=None, Q_bar=None):
        self.token_size = token_size
        self.Q = Q
        self.Q_bar =Q_bar

        if self.token_size != self.Q.shape[-1]:
            raise Exception('Token size %d does not match the matrix shape %s!' % (self.token_size, self.Q.shape))
    ...
    def __call__(self, sequences:torch.Tensor, timesteps:torch.Tensor, seq_mask:torch.Tensor):
        ...
        Outputs:
            src: source masked sequences (model input); (N, L)
            src_onehot: one hot encoding of masked sequences; (N, L, token_size + 1)
            onehot: one hot encoding of original sequences; (N, L, token_size + 1)
            q_x : forward transition probabilities; (N, L, token_size)
        """
        ...
        ### one hot encoding
        onehot = torch.nn.functional.one_hot(sequences, self.token_size + 1).to(torch.double)  # (N, L, token_size + padding_token)
        ### transition
        q_x = torch.bmm(onehot[:,:,:-1], torch.stack([self.Q_bar[t] for t in timesteps]).to(device))  # (N, L, token_size)
        q_x[seq_mask == 0] = 1 / self.token_size  # avoid zero probability for all tokens 
        src = torch.multinomial(q_x.reshape(-1, q_x.shape[-1]), num_samples=1).reshape(N, L) # (N, L)
       
        q_x[seq_mask == 0] = 0
        q_x = q_x.to(torch.double)
        src[seq_mask == 0] = sequences[seq_mask == 0]
        src = src.to(torch.long)
        src_onehot = torch.nn.functional.one_hot(src, self.token_size + 1).to(torch.double)

        return src, src_onehot, onehot, q_x
