...
### adapt to diffab
from diffab.utils.protein.constants import ressymb_order # 'ACDEFGHIKLMNPQRSTVWYX'
...
class TokenizerSingleSeq(Tokenizer):
    """Convert between strings and index (adapted to diffab by SZ)"""
    def __init__(self, 
        protein_alphabet=ressymb_order + '!#', 
        pad='!', 
        mask='#', 
        all_aas=ressymb_order,
        path_to_blosum=None
        ):
        self.alphabet = list("".join(protein_alphabet))
        self.all_aas = list("".join(all_aas))
        ...
        if path_to_blosum is not None:
            self.matrix = loadMatrix(path_to_blosum)
            self.matrix = {key : self.matrix[key] 
                              for key in self.matrix if self.pair_check(key)} 
            self.matrix_dict = dict(self.matrix)

    def pair_check(self, key):
        """Check whether the pair works for the token set."""
        for char in key:
            if not char in self.all_aas:
                return False
        return True
    ...
    def q_blosum(self):
        ...
        for i, row in enumerate(new_q):
            for j, value in enumerate(row):
                ...
                if key[0] in self.a_to_i and key[1] in self.a_to_i:
                    new1, new2 = [self.a_to_i[k] for k in key]
                    new_q[new1, new2] = q[ind1, ind2]
        #IF TRAINING SEQUENCES - DROP GAP
        return new_q
