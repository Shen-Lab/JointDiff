...
def get_optimizer(cfg, model):
    if cfg.type == 'adam':
        return torch.optim.Adam(
            #model.parameters(),
            filter(lambda p: p.requires_grad, model.parameters()),  # by SZ
            lr=cfg.lr,
...
def log_losses(out, it, tag, logger=BlackHole(), writer=BlackHole(), others={}):
    ...
    for k, v in out.items():
        ...
        if v.shape: ## add by SZ for multi-GPU
            v = v.mean() ## 
...
def sum_weighted_losses(losses, weights):
    ...
    for k in losses.keys():
        # if weights is None:
        if weights is None or k not in weights:  # by SZ on 6/9/24
            loss = loss + losses[k]
        else:
            loss = loss + weights[k] * losses[k]

    # add by SZ for multi-GPU
    if loss.shape: ##
        loss = loss.mean() ##
