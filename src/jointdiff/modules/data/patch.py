...
import jointdiff.modules.data.constants as constant

###### add by SZ for coor centralization ######
@register_transform('coor_central')
class CoorCentral(object):
    ...
    def __call__(self, data):
        CA_points = data['pos_heavyatom'][:, constants.BBHeavyAtom.CA]    # (L, 3)
        data = self._center(
            data,
            origin = CA_points.mean(dim=0)
        )
        return data
...
class PatchAroundAnchor(object):
    ...
    def __call__(self, data):        
        ...
        antibody_mask = torch.logical_not(antigen_mask) # SZ: False for antigen part
        ...
        dist_anchor = torch.cdist(pos_alpha, anchor_points).min(dim=1)[0]    # (L, ); SZ: the smallest dist from each CA to the anbchor CAs
        initial_patch_idx = torch.topk(
            dist_anchor,
            k = min(self.initial_patch_size, dist_anchor.size(0)),
            largest=False,
        )[1]   # (initial_patch_size, ) # SZ: k closest CA
