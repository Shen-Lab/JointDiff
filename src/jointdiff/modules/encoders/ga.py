...
from jointdiff.modules.common.layers import mask_zero, LayerNorm
from jointdiff.modules.data.constants import BBHeavyAtom
...
class GABlock(nn.Module):
    ...
    def __init__(self, 
        node_feat_dim, pair_feat_dim, value_dim=32, query_key_dim=32, 
        num_query_points=8, num_value_points=8, num_heads=12, bias=False
    ):
        super().__init__()
        ...
        self.proj_query = nn.Linear(
            node_feat_dim, query_key_dim * num_heads, bias=bias
        )
        self.proj_key = nn.Linear(
            node_feat_dim, query_key_dim * num_heads, bias=bias
        )
        self.proj_value = nn.Linear(
            node_feat_dim, value_dim * num_heads, bias=bias
        )

        ###### Pair ######
        self.proj_pair_bias = nn.Linear(
            pair_feat_dim, num_heads, bias=bias
        )
    ...
    def _node_logits(self, x):
        ...
        logits_node = (
            query_l.unsqueeze(2) * key_l.unsqueeze(1) *
            (1 / np.sqrt(self.query_key_dim))
        ).sum(-1)  # (N, L, L, num_heads)
        return logits_node
    ...
    def _spatial_logits(self, R, t, x):
        ...
        query_points = local_to_global(R, t, query_points)  # (N, L, n_heads * n_pnts, 3)
        ...
        key_points = local_to_global(R, t, key_points)  # (N, L, n_heads * n_pnts, 3)
        key_s = key_points.reshape(N, L, self.num_heads, -1)  # (N, L, n_heads, n_pnts*3)

        ###### Q-K Product ######
        sum_sq_dist = (
            (query_s.unsqueeze(2) - key_s.unsqueeze(1)) ** 2
        ).sum(-1)  # (N, L, L, n_heads)
    ...
    def _spatial_aggregation(self, alpha, R, t, x):
        ...
        value_points = local_to_global(
            R, t, 
            value_points.reshape(
                N, L, self.num_heads, self.num_value_points, 3
            ),
        ) # (N, L, n_heads, n_v_pnts, 3)
