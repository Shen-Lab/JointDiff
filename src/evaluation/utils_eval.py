...
def TM_score(pdb_1, pdb_2, with_RMSD = False):
    ...
    ###### first align ######
    command_1 = './TMalign_cpp ' + pdb_1 + ' ' + pdb_2 #+ ' -a'
    ...
    if "(if normalized by length of Chain_1" in out_1:
        ...
        print('TMalign failed for %s and %s!' % (pdb_1, pdb_2))
        print(out_1)
        tms_1 = None
    ### RMSD
    ...
    ###### second_align ######
    command_2 = './TMalign_cpp ' + pdb_2 + ' ' + pdb_1 #+ ' -a'
    ...
    if "(if normalized by length of Chain_1" in out_2:
        ...
        print('TMalign failed for %s and %s!' % (pdb_2, pdb_1))
        print(out_2)
        tms_2 = None
    ### RMSD
...
def TM_score_asym(pdb_1, pdb_2, with_RMSD = False):
    ...
    command = './TMalign_cpp ' + pdb_1 + ' ' + pdb_2
    ...
    for chain in structure:
        for resi in chain:
            ...
            for atom in resi:
                ...
                if atom_name == 'CA':
                    atom_coor = atom.coord
                    coor_list.append(atom_coor)
                    if resi_name in RESIDUE_reverse_dict:
                        seq += RESIDUE_reverse_dict[resi_name]
                    else:
                        seq += 'X'
                    break

    if coor_list:
        return np.vstack(coor_list), seq
    else:
        print('No CA atom detected!')
        return None, None


def TMscore_cal_tmtool(pdb_1, pdb_2, with_RMSD = True):
    coords1, seq1 = coor_load(pdb_1)
    coords2, seq2 = coor_load(pdb_2)

    res_1 = tm_align(coords1, coords2, seq1, seq2)
    tmscore_1 = res_1.tm_norm_chain1
    rmsd_1 = res_1.rmsd

    res_2 = tm_align(coords2, coords1, seq2, seq1)
    tmscore_2 = res_2.tm_norm_chain1
    rmsd_2 = res_2.rmsd
    
    if with_RMSD:
        return (tmscore_1 + tmscore_2) / 2, (rmsd_1 + rmsd_2) / 2
    else:
        return (tmscore_1 + tmscore_2) / 2
