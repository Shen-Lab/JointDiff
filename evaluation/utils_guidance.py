...
def pdb_write(coor, path, seq = None, chain = 'A',
    ...
    if seq is not None and coor.shape[0] != len(seq):
        raise Exception('Error! The size of the strutctue and the sequence do not match! (%d and %d)'%(coor.shape[0], len(seq)))
    elif coor.shape[1] != len(atom_list):
        raise Exception('Error! The size of the resi-wise coor and the atom_num do not match! (%d and %d)'%(coor.shape[1], len(atom_list)))
    else:
...
class FitnessGrad(nn.Module):
    def __init__(self, version = 'ESM-1b', input_voc_set = ressymb_order):
        ...
        #                            requires_grad=False)
        self.model.emb_inv = nn.Parameter(emb_W.T, requires_grad=False) # rather than psedo-inverse, directly apply the transposed mat
        self.model.eval()
        ...
        self.seq_trans_mat = nn.Parameter(self.seq_trans_mat, requires_grad=False) 
        self.grad_sele_mat = nn.Parameter(self.seq_trans_mat.T, requires_grad=False) # (esm_voc_size, input_voc_size)
    ...
    def forward(self, seq, mask = None, with_grad = True, seq_transform = True, with_padding = False):
        ...
        fitness, grad = self.model.fitness_cal(seq, with_grad = with_grad, with_padding = with_padding, grad_sele = self.grad_sele_mat)
        # fitness: (B,)
        # grad: (B, L_max, 21)
        ...
        for line in lines:
            if line[0:4] == 'ATOM' and ''.join(line[12:16].split(' ')) == 'CA':
                ...
                if chain_pre is None or chain != chain_pre:
                    ...
                    coor_dict[chain_idx] = np.vstack([coor_dict[chain_idx], np.array([[x, y, z]])])
            ...
            for resi_1, coor_1 in enumerate(coor_dict[chain_idx_1]):
                ...
                for resi_2, coor_2 in enumerate(coor_dict[chain_idx_2]):
                    ...
                    if (chain_idx_1 < chain_idx_2 or resi_1 < resi_2) and dist <= threshold:
                        contact_list.append((chain_idx_1, resi_1 + 1, chain_idx_2, resi_2 + 1))
    ...
    for i in range(start_idx):
        ...
        for j in range(i):
            ...
            if contact_map[i, j] == 1:
                contact_list.append((chain_idx_1, resi_idx_1, chain_idx_2, resi_idx_2))
...
def sbm_getCAModel(structure_file, contact_file,
    ...
    if default_parameters and default_forces and create_system:
        sbm.createSystemObject(minimize=minimize, check_bond_distances=False, check_large_forces = check_large_forces)

    return sbm

def force_and_energy(pdb_path = None, temp_dir = './',
                     contact_thre = 12, with_contact = True, get_force = True, get_energy = True, 
                     name_tag = None, sum_result = False, force_pad = None, device = 'cpu',
                     with_pdb_write = False, coor = None, seq = None, mask = None, atom_list = ['CA'], pdb_remove = True):
    """
    Args:
    ...
    if with_pdb_write:
        ...
        ### pdb path
        path_idx = 1
        pdb_path = temp_dir + '/temp%s_%d.pdb'%(name_tag, path_idx)  # temperory file
    ...
    ### predefined contacts
    if type(with_contact) == str:
        if os.path.exists(with_contact) and (not empty_file(with_contact)):
            try:
                sbmCAModel = sbm_getCAModel(pdb_path, with_contact)
            except Exception as e:
                print('Error!', e)
                print(pdb_path)
                print(with_contact)
        else:
            print('Warning! Contact file %s does not exists or empty!'%with_contact)
            sbmCAModel = sbm_getCAModel(pdb_path, None)
    ### dynamic contacts
    elif with_contact:
        ### contact file path ###
        path_idx = 1
        temp_file = temp_dir + '/temp%s_%d.contact'%(name_tag, path_idx)  # contact file
        while os.path.exists(temp_file):
            path_idx += 1
            temp_file = temp_dir + '/temp%s_%d.contact'%(name_tag, path_idx)  # contact file

        ### generate the contact file ###
        dist_mat, contact_map = CA_contact_gen(pdb_path, temp_file, threshold = contact_thre)
        if empty_file(temp_file):
            print('Contact file %s for %s is empty. Set to be None.'%(temp_file, pdb_path))
            sbmCAModel = sbm_getCAModel(pdb_path, None)
        else:
            sbmCAModel = sbm_getCAModel(pdb_path, temp_file)
        os.remove(temp_file)  # remove the temparay file
    ### no contacts
    else:
        sbmCAModel = sbm_getCAModel(pdb_path, None)
    ...
    for i,n in enumerate(sbmCAModel.forceGroups):
        state = simulation.context.getState(getEnergy=get_energy, getForces=True, groups={i})
        if get_force:
            ### snm unit: kilojoule/(nanometer*mole); 10 kilojoule/(nanometer*mole) = 1 kilojoule/(A*mole)
            force_dict[n] = (quantity_to_tensor(state.getForces()) * 0.1).to(device) 
        if get_energy:
            energy_dict[n] = (torch.tensor(state.getPotentialEnergy().value_in_unit(unit.kilojoules_per_mole))).to(device)

    ###### add up the results and add the padding
    if get_force:
        ### add up forces
        if sum_result:
            force_dict = {'all': torch.sum(torch.stack([force_dict[key] for key in force_dict.keys()]), dim = 0)}
        #else:
        #    force_dict = {key: torch.unsqueeze(force_dict[key], dim=0) for key in force_dict.keys()}
        
        ### add padding
        if force_pad is not None: ### shape match; force_dict: {key: (L_single, 3)} -> {key: (1, L, 3)}
            force_dict = {key: F.pad(force_dict[key], (0, 0, 0, force_pad - force_dict[key].shape[0]), "constant", 0).unsqueeze(0).to(device)
                              for key in force_dict.keys()} 

        #for key in force_dict.keys():
        #    print(key, force_dict[key].shape)

    if get_energy and sum_result:
        energy_dict = {'all': torch.sum(torch.stack([energy_dict[key] for key in energy_dict.keys()]), dim = 0)}

    ###### for the invalid contact file
    if not sum_result and with_contact and not 'LJ 12-10 Contact Energy' in sbmCAModel.forceGroups:
        for i,n in enumerate(sbmCAModel.forceGroups):
            key_sele = n
            break
        if get_force:
            force_dict['LJ 12-10 Contact Energy'] = (torch.ones(force_dict[key_sele].shape) * torch.nan).to(device)
        if get_energy:
            energy_dict['LJ 12-10 Contact Energy'] = (torch.ones(energy_dict[key_sele].shape) * torch.nan).to(device)

    ###### remove temperary pdb file 
    if with_pdb_write and pdb_remove:
        os.remove(pdb_path)

    return force_dict, energy_dict


def force_and_energy_multi(pdb_path_list = None, 
                           with_contact = True, contact_thre = 12, get_force = True, get_energy = True, sum_result = True, force_pad = 200,
                           temp_dir = './', name_tag = None, multithread = True, device = 'cpu',
                           with_pdb_write = False, coor_list = None, seq_list = None, mask_list = None, atom_list = ['CA'], pdb_remove = True):
    """
    ...
    if with_pdb_write:
        N = coor_list.shape[0]  # sample amounts
    else:
        N = len(pdb_path_list)
    ################### multi-thread for parallel computing ###################
    if multithread:
        ###### thread prepare
        Thread_list = []
        for i in range(N):
            ...
            ### contact file
            if type(with_contact) == list:
                with_contact_temp = with_contact[i]
            else:
                with_contact_temp = with_contact

            Thread_list.append(ThreadWithReturnValue(target=force_and_energy, args = (pdb_path,
                                                                                      temp_dir, 
                                                                                      contact_thre,
                                                                                      with_contact_temp, 
                                                                                      get_force, 
    ...
    ################### single-thread sequential computing ###################
        ...
        for i in range(N):
            ...
            force_dict_all[i], energy_dict_all[i] = force_and_energy(pdb_path = pdb_path, temp_dir = temp_dir,
                                                        contact_thre = contact_thre, with_contact = with_contact_temp,
                                                        get_force = get_force, get_energy = get_energy, name_tag = name_tag_thread,
                                                        sum_result = sum_result, force_pad = force_pad, device = device,
                                                        with_pdb_write = with_pdb_write, coor = coor, seq = seq, mask = mask, 
                                                        atom_list = atom_list, pdb_remove = pdb_remove)

    ################### results summarization ###################
    if get_force and force_dict_all:
        #N = len(force_dict_all.keys())
        key_set = force_dict_all[0].keys()
        force_dict_all = {key: torch.cat([force_dict_all[i][key] for i in range(N)], dim = 0)
                              for key in key_set}
        #for key in key_set:
        #    print(key, force_dict_all[key].shape)
    else:
        force_dict_all = None

    if get_energy and energy_dict_all:
        #N = len(energy_dict_all.keys())
...
###################################################################################
# For energy and Force calculation (with original openmm)
###################################################################################

def force_and_energy_openmm(
    pdb_path:str,
    force_field:Tuple[str, str] = ('amber14-all.xml', 'amber14/tip3pfb.xml'),
    get_force:bool = True, 
    get_energy:bool = True,
    force_pad:int = None,
    device = 'cpu'
) -> Tuple[torch.Tensor, torch.Tensor]:
    """
    Args:
        pdb_path: path of the pdb file; if with_pdb_write, then will reassign this term for coor 
        force_pad: the output length to be padded

    Output: 
        force: protential force of the given structure.
        energy: potential enrgy of the given structure.
    """
    if not (get_force or get_energy):
        print('Warning! Neither force nor energy calculation is required!')
        return None, None
    ...
    forcefield = ForceField(force_field[0], force_field[1])
    ...
    state = simulation.context.getState(getEnergy=get_energy, getForces=get_force)

    energy = (torch.tensor(state.getPotentialEnergy().value_in_unit(unit.kilojoules_per_mole))).to(device) if get_energy else None
    ### snm unit: kilojoule/(nanometer*mole); 10 kilojoule/(nanometer*mole) = 1 kilojoule/(A*mole)
    if get_force:
        force = (quantity_to_tensor(state.getForces()) * 0.1).to(device)
        if force_pad is not None: ### shape match; force_dict: {key: (L_single, 3)} -> {key: (1, L, 3)}
            force = F.pad(force, (0, 0, 0, force_pad - force.shape[0]), "constant", 0).unsqueeze(0).to(device)
    else:
        force = None
...
class Guidance_cal(nn.Module):
    def __init__(self, 
        ...
        if with_fitness_guide:
            self.fitness_module = FitnessGrad(version = esm_version, input_voc_set = input_voc_set).to(device)

    def energy_guide(self, coor, seq, mask = None, atom_list = ['CA'], 
                     temp_dir = './', contact_thre = 12, with_contact = True, 
                     get_force = True, get_energy = True, sum_result = True, multithread = True):
        """
        Args:
        ...
        ###### energy cal
        force_dict_all, energy_dict_all  = force_and_energy_multi(pdb_path_list = None,
...
if __name__ == "__main__":
    ...
    dist_mat_2, contact_map_2 = contact_gen_torch(pdb_path, out_path_2, threshold = 12, atom_sele = 'CA')
    ...
    # pdb_path_list = [pdb_path + p for p in pdb_list]
